...
vi.mock("../../mobile/convex/_generated/api", () => ({
  api: { library: { episodes: { setEpisodeState: "save" }, items: { listTagSuggestions: "tags" } } },
}));
...
it("locks episode controls during autosave and restores them after failure", async () => {
  ...
  mocks.save.mockReturnValue(new Promise((_, reject) => { rejectSave = reject; }));
  render(
    <EpisodeDialog episode={{ itemId: "item", title: "Show", seasonName: "Season 1", season: 1, episode: 1, name: "Pilot", rating: 4, watched: true }}>
  ...
  expect(screen.getByRole("alert")).toHaveTextContent("Couldn’t update this episode");
});
